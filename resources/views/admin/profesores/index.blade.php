<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.profesores.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                + Nuevo Profesor
            </a>
        </div>
    </x-slot>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($profesores as $profesor)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-600">
                                                {{ $profesor->classroomsAsTeacher->count() }} clase(s)
                                            </div>
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                                            No hay profesores registrados. <a href="{{ route('admin.profesores.create') }}" class="text-indigo-600 hover:text-indigo-800">Crear el primero</a>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
