                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.classrooms.update', $classroom) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="subject" class="block mt-1 w-full" type="text" name="subject" :value="old('subject', $classroom->subject)" />
                        ...
                        <div>
                            ...
                            <select id="shift" name="shift" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1">
                                ...
                                <option value="mañana" {{ old('shift', $classroom->shift) === 'mañana' ? 'selected' : '' }}>Mañana</option>
                                <option value="tarde" {{ old('shift', $classroom->shift) === 'tarde' ? 'selected' : '' }}>Tarde</option>
                                <option value="noche" {{ old('shift', $classroom->shift) === 'noche' ? 'selected' : '' }}>Noche</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="teacher_id" name="teacher_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1" required>
                                ...
                                @foreach($profesores as $profesor)
                                    <option value="{{ $profesor->id }}" {{ old('teacher_id', $classroom->teacher_id) == $profesor->id ? 'selected' : '' }}>
                                        {{ $profesor->name }}
                                    </option>
                                @endforeach
                            </select>
